...
class RandomResize(object):
    def __init__(self, min_size_rate, max_size_rate=None):
    ...
    def __call__(self, image, target):
        ...
        rate=random.uniform(self.min_size_rate,self.max_size_rate)
        h = int(image.size[0]*rate)
        w=int(image.size[1]*rate)
        # 这里size传入的是int类型，所以是将图像的最小边长缩放到size大小
        image = F.resize(image, (h,w))
        # 这里的interpolation注意下，在torchvision(0.9.0)以后才有InterpolationMode.NEAREST
        # 如果是之前的版本需要使用PIL.Image.NEAREST
        target = F.resize(target, (h,w), interpolation=T.InterpolationMode.NEAREST)
        return image, target
